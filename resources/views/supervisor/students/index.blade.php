...
<tr>
    ...
    <td>
        @php
            ...
            $project = \App\Models\Project::where('student_id', $student->id)->first();
        @endphp
    ...
    <td>
        @if($student->project)
    <a href="{{ route('supervisor.projects.show', $student->project->id) }}" class="btn btn-sm btn-primary">
        <i class="fas fa-eye"></i> Voir le projet
    </a>
@else
    @php
        // Essayer de trouver un projet associé directement via la base de données
        $directProject = \App\Models\Project::where('student_id', $student->id)->first();
    @endphp
    
    @if($directProject)
        <a href="{{ route('supervisor.projects.show', $directProject->id) }}" class="btn btn-sm btn-primary">
            <i class="fas fa-eye"></i> Voir le projet
        </a>
    @else
