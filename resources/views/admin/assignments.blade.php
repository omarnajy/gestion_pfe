                        ...
                        <ul class="nav nav-tabs card-header-tabs" id="assignmentsTabs" role="tablist">
                            <li class="nav-item" role="presentation">
                                <button class="nav-link active" id="current-tab" data-bs-toggle="tab"
                                    ...
                                    aria-selected="true">Affectations en cours</button>
                            </li>
                            <li class="nav-item" role="presentation">
                                <button class="nav-link" id="unassigned-tab" data-bs-toggle="tab"
                                    ...
                                    aria-selected="false">Étudiants non affectés</button>
                            </li>
                            <li class="nav-item" role="presentation">
                                <button class="nav-link" id="supervisors-tab" data-bs-toggle="tab"
                                    data-bs-target="#supervisors" type="button" role="tab" aria-controls="supervisors"
                                    aria-selected="false">Encadreurs</button>
                            </li>
                        </ul>
                    </div>
                    <div class="card-body">
                                            ...
                                            @forelse($currentAssignments ?? [] as $assignment)
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        @include('admin.partials.edit-assignment-modal', [
                                                            'assignment' => $assignment,
                                                        ])
                                                        @include('admin.partials.delete-assignment-modal', [
                                                            'assignment' => $assignment,
                                                        ])
                                                    </td>
                                                </tr>
                                                    ...
                                                    <td colspan="7" class="text-center py-3 text-muted">
                                                        <div class="py-4">
                                                            ...
                                                            <button type="button" class="btn btn-sm btn-primary"
                                                                data-bs-toggle="modal" data-bs-target="#newAssignmentModal">
                                    ...
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            @forelse($unassignedStudents ?? [] as $student)
                                                @php
                                                    // Récupérer le projet de l'étudiant s'il en a un
                                                    $studentProject = \App\Models\Project::where(
                                                        'student_id',
                                                        $student->id,
                                                    )->first();
                                                @endphp
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if ($studentProject)
                                                            <a href="{{ route('admin.projects.show', $studentProject->id) }}"
                                                                class="text-decoration-none">
                                                                <span
                                                                    class="badge bg-success">{{ $studentProject->title }}</span>
                                                            </a>
                                                        @else
                                                            <span class="badge bg-secondary">Aucun projet</span>
                                                        @endif
                                                    ...
                                                    <td>
                                                        <button type="button" class="btn btn-sm btn-primary"
                                                            data-bs-toggle="modal"
                                                            data-bs-target="#assignStudentModal{{ $student->id }}">
                                                            <i class="fas fa-user-plus me-1"></i> Affecter
                                                        </button>

                                                        <!-- Modal pour l'affectation -->
                                                        @include('admin.partials.assign-student-modal', [
                                                            'student' => $student,
                                                        ])
                                                    </td>
                                                </tr>
...
$assignedStudentsCount = $currentAssignments
    ->where('supervisor_id', $supervisor->id)
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="{{ route('admin.assignments') }}?supervisor_id={{ $supervisor->id }}"
                                                ...
                                                <tr>
                                                    <td colspan="7" class="text-center py-3 text-muted">
                                                        <div class="py-4">
                                                            ...
                                                            <a href="{{ route('admin.users.create', 'supervisor') }}"
    ...
    <div class="modal fade" id="newAssignmentModal" tabindex="-1" aria-labelledby="newAssignmentModalLabel"
        aria-hidden="true">
                            ...
                            <select class="form-select" id="student_id" name="student_id" required>
                                ...
                                @foreach ($unassignedStudents ?? [] as $student)
                                    <option value="{{ $student->id }}">{{ $student->name }}
                                        ({{ $student->field ?? 'Filière non définie' }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <select class="form-select" id="supervisor_id" name="supervisor_id" required>
                                ...
                                @foreach ($availableSupervisors ?? [] as $supervisor)
                                    <option value="{{ $supervisor->id }}">{{ $supervisor->name }}
                                        ({{ $supervisor->specialty ?? 'Spécialité non définie' }})
                                        -
                                        {{ $supervisor->students_count ?? 0 }}/{{ $supervisor->max_students ?? 5 }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="alert alert-info">
                            <i class="fas fa-info-circle me-1"></i> Si l'étudiant a déjà un projet, celui-ci sera
                            automatiquement assigné à l'encadreur sélectionné.
                        </div>
                    </div>
