                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        @foreach ($projects as $project)
                                    ...
                                    <div class="modal-content">
                                        <form action="{{ route('admin.projects.validate', $project->id) }}" method="POST">
                                            ...
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="validateModalLabel{{ $project->id }}">
                                                    Validation finale du projet</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                    aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <p>Êtes-vous sûr de vouloir valider définitivement le projet
                                                    <strong>{{ $project->title }}</strong> ?
                                                </p>
                                                <p>Cette action marquera le projet comme finalisé dans le système.</p>
                                                <div class="mb-3">
                                                    <label for="validation_comment" class="form-label">Commentaire
                                                        (optionnel)
                                                    </label>
                            ...
                            <div class="modal fade" id="assignSupervisorModal{{ $project->id }}" tabindex="-1"
                                aria-labelledby="assignSupervisorModalLabel{{ $project->id }}" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form action="{{ route('admin.assignments.store') }}" method="POST">
                                            ...
                                            @if ($project->student)
                                                <input type="hidden" name="student_id"
                                                    value="{{ $project->student->id }}">

                                                <div class="modal-header">
                                                    <h5 class="modal-title"
                                                        id="assignSupervisorModalLabel{{ $project->id }}">Assigner un
                                                        encadreur</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                        aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <p>Assigner un encadreur au projet
                                                        <strong>{{ $project->title }}</strong> de l'étudiant
                                                        <strong>{{ $project->student->name }}</strong>.
                                                    </p>

                                                    <div class="mb-3">
                                                        <label for="supervisor_id" class="form-label">Sélectionner un
                                                            encadreur</label>
                                                        <select class="form-select" id="supervisor_id"
                                                            ...
                                                            @foreach ($supervisors as $supervisor)
                                                                <option value="{{ $supervisor->id }}">
                                                                    {{ $supervisor->name }}
                                                                    ({{ $supervisor->active_projects_count }} projets en
                                                                    cours)
                                                                </option>
                                                            @endforeach
                                                        </select>
                                            ...
                                            @else
                                                <div class="modal-header">
                                                    <h5 class="modal-title"
                                                        id="assignSupervisorModalLabel{{ $project->id }}">Impossible
                                                        d'assigner un encadreur</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                        aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <div class="alert alert-warning">
                                                        Ce projet n'a pas d'étudiant assigné. Veuillez d'abord assigner un
                                                        étudiant à ce projet avant d'attribuer un encadreur.
                                                    </div>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary"
